...
class FabStock extends StatefulWidget {
  const FabStock({Key? key}) : super(key: key);

  @override
  State<FabStock> createState() => _FabStockState();
}

class _FabStockState extends State<FabStock> with DLL {
  ...
  loadStores() async {
    try {
      ...
      var response = await postRequest(
          apiFabStoreCoding, {"FabricStoreCode": sharedPref.getString("S_FabricStoreCode").toString()});
      ...
      if (response['status'] == "success") {
        ...
        for (var element in customersData) {
          newDropdownMenuItem_CustomersData.add(
            DropdownMenuItem(
              child: Text(element["FabricStoreName"]),
              value: ListItem(
                element["FabricStoreCode"],
                element["FabricStoreName"],
              ),
            ),
          );
        }
                            ...
                            onChanged: (value) async {
                              setState(() {
                                ...
                                _timer = Timer(const Duration(milliseconds: 200), () {
                                  setState(() {
                                    if (value != null) {
                                      if (value.fabricStoreCode != null) {
                                        if (value.fabricStoreCode != "0") {
                                          fabStoreCode = value.fabricStoreCode!;
                                        } else {
                                          fabStoreCode = "ALL";
                                        }
                                      }
  ...
  Future<List<Product>> generateProductList() async {
    ...
    var response =
        await postRequest(apiFabStock, {"WHCode": fabStoreCode});

    for (var item in response["data"]) {
      Product current = Product(
        fabricStoreName: item['FabricStoreName'] ?? "",
        fName: item['FName'] ?? "",
        thType: item['ThType'] ?? "",
        thNo: item['ThNo'] ?? "",
        unit: item['Unit'] ?? "",
  ...
  void buildDataGridRow() {
    dataGridRows = productList.map<DataGridRow>((dataGridRow) {
      return DataGridRow(cells: [
        DataGridCell<String>(columnName: 'FabricStoreName', value: dataGridRow.fabricStoreName),
...
class Product {
  factory Product.fromJson(Map<String, dynamic> json) {
    return Product(
      fabricStoreName: json["FabricStoreName"],
      fName: json["FName"],
      thType: json["ThType"],
      thNo: json["ThNo"],
      unit: json["Unit"],
...
class ListItem {
  String? fabricStoreCode;
  String? fabricStoreName;
  ListItem(this.fabricStoreCode, this.fabricStoreName);
}